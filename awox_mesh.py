...
class AwoxMesh(DataUpdateCoordinator):
    ...
    def _process_command_queue(self):
        while not self._shutdown:
            ...
            _LOGGER.info('[%s] get item from queue', self.mesh_name)
            command = self._queue.get()
            _LOGGER.info('[%s] process 0/%d - %s', self.mesh_name, self._queue.qsize(), command)
            try:
                tries = 0
    ...
    def _call_command(self, command) -> bool:
        ...
        try:
            ...
            if isinstance(command['params'], tuple):
                ...
                result = getattr(self._connected_bluetooth_device, command['command'])(**command['params'])
            _LOGGER.info(f'[{self.mesh_name}] Send command {command["command"]} got result {result}')
        except Exception as e:
            _LOGGER.exception('[%s] Command failed, re-connecting for new attempt - [%s] %s', self.mesh_name, type(e).__name__, e)
            result = None
            failed = True

        _LOGGER.info('[%s] Command result: %s', self.mesh_name, result)
    ...
    async def _async_get_devices_rssi(self):
        ...
        _LOGGER.info(f'[{self.mesh_name}] Scan result: {devices}')
